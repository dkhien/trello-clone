...
function AddTaskDialog() {
  ...
  const [addNewTaskToDB] = useBoardStore((state) => [state.addNewTaskToDB])

  const handleSubmit = (e:FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!newTaskInput) return;
    addNewTaskToDB(newTaskInput, newTaskType, image)
    setImage(null);
    closeModal();
  ...
  return (
    ...
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog as="form" onClose={closeModal} className="relative z-10" onSubmit={handleSubmit}>
        <Transition.Child
          as={Fragment}
        ...
        >
          <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
            ...
            <div className="mt-2">
              <button className='w-full border border-gray-300 rounded-md outline-none p-5 focus-visible:ring-2
              ...
              {image && (
                <Image src={URL.createObjectURL(image)} width={200} height={100} alt="Uploaded Image"
